...
export default async function HomePage({
  ...
  return (
    <main>
      ...
      <Container>
        <div className={styles.hero}>
          ...
          <Stack direction="row" gap="sm" wrap="wrap">
            {settings?.show_cta_projects !== false && (
              <Button as={Link} href={settings?.cta_projects_link || "/projects"} size="lg" iconRight={<ArrowRight size={16} />}>
                {txt(settings?.cta_projects, t("cta_projects"))}
              </Button>
            )}
            {settings?.show_cta_about !== false && (
              <Button as={Link} href={settings?.cta_about_link || "/about"} variant="secondary" size="lg">
      ...
      {/* Featured Projects */}
      {settings?.show_work_section !== false && (
        <Section spacing="xl">
          <Container>
